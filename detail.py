...
db = client.dbsparta
SECRET_KEY = 'parkyeseul' # config 처리 해줘야 합니다. 깃허브에서는 숨겨둡니다.
KEY_SECRET = 'sparta' # 저는 리프레시 토큰과 액세스 토큰의 암호화 코드를 각각 달리 지정해 줬습니다. 사용하시려면 두 암호에 아무 문자열이나 입력해주세요.
    ...
    try:
        ...
        if response["status"] == "OK":
            if len(response["addresses"]) > 0:
                ...
                print("좌표를 찾지 못했습니다")
        return render_template('detail.html', head=renderTag.cur_st_login, museum=museum, img=img, addr_x=x, addr_y=y)
